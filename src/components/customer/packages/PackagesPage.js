...
import { hotelServicesDB, packageServicesDB } from '../../../services/database';
...
const PackagesPage = () => {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      setIsLoading(true);
      // Fetch hotel provider packages
      const hotelPkgs = await hotelServicesDB.getPackages();
      // Fetch package provider packages
      const packageProviderPkgs = await packageServicesDB.getPackages();
      // Merge and set
      setPackages([...hotelPkgs, ...packageProviderPkgs]);
      setIsLoading(false);
    };
    fetchPackages();
  }, []);
